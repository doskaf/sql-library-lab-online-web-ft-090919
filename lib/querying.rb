...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series INNER JOIN books ON books.id = series.author_id INNER JOIN characters ON characters.author_id = authors.id GROUP BY(characters.species) ORDER BY COUNT(characters.species) DESC LIMIT 1;"
end
